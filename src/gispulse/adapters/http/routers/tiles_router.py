...
_WORLD_BOUNDS_4326 = (-180.0, -90.0, 180.0, 90.0)
_TILEJSON_BOUNDS_TTL_S = 300  # 5 min — keeps ST_Extent off the hot path
_bounds_cache: dict[str, tuple[tuple[float, float, float, float], float]] = {}


def _resolve_bounds_4326(
    ds: Dataset,
    engine: SpatialEngine,
) -> tuple[float, float, float, float]:
    """Resolve a dataset's WGS84 bounds, in priority order.

    1. ``dataset.metadata['bounds']`` if a 4-tuple of floats.
    2. PostGIS ``ST_Extent`` of the source table (transformed to 4326).
    3. World bounds.

    PostGIS lookups are cached for 5 min keyed by dataset id (R2.1).
    """
    raw = (ds.metadata or {}).get("bounds")
    ...
    if engine.backend_name != "postgis":
        return _WORLD_BOUNDS_4326

    cache_key = str(ds.id)
    cached = _bounds_cache.get(cache_key)
    if cached is not None and time.time() - cached[1] < _TILEJSON_BOUNDS_TTL_S:
        return cached[0]

    table_name = ds.source_path or ds.name
    sql = f"""
        SELECT
            ST_XMin(b)::float AS xmin, ST_YMin(b)::float AS ymin,
            ST_XMax(b)::float AS xmax, ST_YMax(b)::float AS ymax
        FROM (
            SELECT ST_Transform(ST_SetSRID(ST_Extent(geom), Find_SRID('public', '{table_name}', 'geom')), 4326) AS b
            FROM "{table_name}"
        ) e
    """
    try:
        ...
        logger.exception("ST_Extent lookup failed for dataset %s", ds.id)
        return _WORLD_BOUNDS_4326
    ...
    _bounds_cache[cache_key] = (bounds, time.time())
    return bounds


def _public_base_url(request: Request) -> str:
    """Reconstruct the public base URL respecting reverse-proxy headers.

    Honors ``X-Forwarded-Proto`` and ``X-Forwarded-Host``; falls back to
    ``request.base_url``. Returns the URL without a trailing slash.
    """
...
@router.get("/{collection_id}/tilejson.json")
def get_tilejson(
    collection_id: UUID,
    request: Request,
    ...
    """Serve a TileJSON 3.0 document for the given dataset.

    Spec: https://github.com/mapbox/tilejson-spec/tree/master/3.0.0

    Lets MapLibre / deck.gl / QGIS / ArcGIS Online discover the MVT URL
    template, bounds, center, and vector layer schema in a single GET.
    """
    ds = _get_dataset_or_404(collection_id, repo)
    bounds = _resolve_bounds_4326(ds, engine)
    center_lon = (bounds[0] + bounds[2]) / 2.0
    center_lat = (bounds[1] + bounds[3]) / 2.0

    base = _public_base_url(request)
    tile_url = f"{base}/tiles/{collection_id}/{{z}}/{{x}}/{{y}}.mvt"

    layers_meta = (ds.metadata or {}).get("layers")
    if isinstance(layers_meta, list) and layers_meta:
        vector_layers = [
            {
                "id": str(lyr.get("name") or lyr.get("id") or ds.name),
                "fields": dict(lyr.get("fields") or {}),
            }
            for lyr in layers_meta
            if isinstance(lyr, dict)
        ]
    else:
        vector_layers = [{"id": ds.name or str(collection_id), "fields": {}}]

    return {
        "tilejson": "3.0.0",
        "name": ds.name,
        "description": (ds.metadata or {}).get("description", ""),
        "tiles": [tile_url],
        "minzoom": 0,
        "maxzoom": 22,
        "bounds": list(bounds),
        "center": [center_lon, center_lat, 6],
        "scheme": "xyz",
        "format": "pbf",
        "vector_layers": vector_layers,
        "attribution": (ds.metadata or {}).get("attribution", "GISPulse"),
    }
