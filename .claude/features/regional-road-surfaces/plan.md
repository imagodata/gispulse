...
- [x] Classify reconstructed GRB faces from explicit upstream evidence (`classify_grb_faces`, PR #496). Axis-evidence-only, fail-closed, evidence-cited; validated live on the Gand bbox after **two rounds** of adversarial review — 36 `carriageway_paved` / 85 `unmapped` of 121 faces (see plugin README `## Validation`). Round 1 fixed an axis-graze false positive and unsplit corridors wrongly excluded; round 2 found that a two-pass Wcz-based `sidewalk` design (round 1's fix for the same inversion bug) was itself invertible — a carriageway split by a transversal Wcz line reads as `sidewalk` on its uncovered half — and that a symmetric numerator/denominator "fix" introduced a worse regression than the asymmetry it corrected. `sidewalk` was removed entirely rather than patched a third time; coverage dropped from 89/121 (with the broken `sidewalk` path) to 36/121, deliberately, fail-closed. See "Why there is no `sidewalk` class yet" in the plugin README.
